...
const Pembayaran = () => {
  ...
  const handleCheckboxChange = (tagihanId) => {
    // Check if the row is already selected
    if (selectedRows.includes(tagihanId)) {
      // If selected, remove from the list
      setSelectedRows(selectedRows.filter((id) => id !== tagihanId));
    } else {
      // If not selected, add to the list
      setSelectedRows([...selectedRows, tagihanId]);
    }
  };
  ...
  const getAll = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/tagihan/pembayaran`,
        {
                ...
                {totalTagihan.length > 0 ? (
                  totalTagihan.map((tagihan, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <a href={"/sistem_pondok/pembayaran/" + tagihan.transaksi_id}>
